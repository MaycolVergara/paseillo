    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="lg:col-span-2 bg-white dark:bg-gray-900 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 p-6 min-h-[400px]">
            <div class="flex justify-between items-center border-b border-gray-100 pb-4 mb-6">
                ...
                <span id="texto-estado" class="text-xs font-bold text-gray-500 bg-gray-100 px-3 py-1 rounded-lg">Mesas actuales: {{ $table_view->count() }}</span>
            </div>
            ...
            <div id="contenedor-mesas" class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4">
                ...
                @if(isset($table_config) && count($table_config) > 0)
                    @foreach($table_config as $table)
                        <button class="aspect-square flex flex-col items-center justify-center gap-1 rounded-2xl border-2 {{ $table->status == 'disponible' ? 'border-emerald-100 bg-emerald-50 text-emerald-600' : 'border-red-100 bg-red-50 text-red-600' }} hover:scale-105 transition-all shadow-sm">
                            {{-- Cambiado: numero_mesa -> table_number --}}
                            <span class="text-2xl font-black">{{ str_pad($table->table_number, 2, '0', STR_PAD_LEFT) }}</span>
