    ...
    <div
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div class="flex flex-1 max-w-md items-center gap-3">
                ...
                <a href="{{ url('/dashboard/productoRegistro') }}"
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div
                                    ...
                                    @php
                                        $cat = $categorias->where('id_categoria', $producto->id_categoria)->first();

                                        $textoAnalizar = $cat ? strtolower($cat->nombre_categoria) : strtolower($producto->nombre_producto);
                                        $emoji = '🍽️';

                                        if (str_contains($textoAnalizar, 'pizza')) {
                                            $emoji = '🍕';
                                        } elseif (str_contains($textoAnalizar, 'hamburguesa') || str_contains($textoAnalizar, 'burger')) {
                                            $emoji = '🍔';
                                        } elseif (str_contains($textoAnalizar, 'bebida') || str_contains($textoAnalizar, 'gaseosa') || str_contains($textoAnalizar, 'refresco')) {
                                            $emoji = '🥤';
                                        } elseif (str_contains($textoAnalizar, 'krispy') || str_contains($textoAnalizar, 'pollo') || str_contains($textoAnalizar, 'broaster')) {
                                            $emoji = '🍗';
                                        } elseif (str_contains($textoAnalizar, 'salchipapa') || str_contains($textoAnalizar, 'papas')) {
                                            $emoji = '🍟';

                                        } elseif (str_contains($textoAnalizar, 'salchipapa') || str_contains($textoAnalizar, 'alitas')) {
                                            $emoji = '🍗';
                                        }
                                    @endphp
                                ...
                                <div>
                                    <p class="text-sm font-bold text-gray-900 dark:text-white texto-buscar">{{ $producto->nombre_producto }}</p>
                                    <p class="text-[11px] text-gray-400 line-clamp-1 max-w-[150px] italic">{{ $producto->descripcion_producto }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <span
                                    class="text-sm font-black text-emerald-600 dark:text-emerald-400">S/ {{ number_format($producto->precio, 2) }}</span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            @php $cat = $categorias->where('id_categoria', $producto->id_categoria)->first(); @endphp
                            <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wider {{ $cat ? 'bg-orange-100 text-orange-600 dark:bg-orange-900/30' : 'bg-gray-100 text-gray-500' }}">
                                {{ $cat ? $cat->nombre_categoria : 'Sin Categoria' }}
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <span
                                        class="w-1.5 h-1.5 rounded-full {{ $producto->fecha_entrega ? 'bg-blue-500' : 'bg-amber-500' }}"></span>
                                <p class="text-[10px] font-bold text-gray-500 dark:text-gray-400">{{ $producto->fecha_entrega ?? 'Inmediata' }}</p>
                            </div>
                        </td>
                            ...
                            <div class="inline-block text-left group/menu relative">
                                ...
                                <div
                                    ...
                                    <div class="p-1.5 space-y-1">
                                        <a href="{{ url('/dashboard/productos/' . $producto->id_producto . '/editar') }}"
                                        ...
                                        <form
                                                action="{{ url('/dashboard/productos/' . $producto->id_producto . '/eliminar') }}"
