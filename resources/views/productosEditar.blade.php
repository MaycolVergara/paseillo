...
@section('content')
    ...
    <div
        ...
        <div class="px-8 py-6 border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/50">
            ...
            <p class="text-sm text-gray-500 mt-1">Completa los datos requeridos para registrar el plato en el
                sistema.</p>
        </div>

        <form action="{{ url('/dashboard/productos/' . $producto->id_producto . '/actualizar') }}" method="POST" enctype="multipart/form-data" class="p-8">              class="p-8">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="nombre_producto" required placeholder="Ej. Pizza Americana Familiar"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="relative">
                            <select name="id_categoria" required
                                ...
                                @foreach($categorias as $categoria)
                                    <option
                                        value="{{ $categoria->id_categoria }}" {{ $producto->id_categoria == $categoria->id_categoria ? 'selected' : '' }}>
                                        {{ $categoria->nombre_categoria }}
                                    </option>
                                @endforeach
                            </select>
                            {{-- Icono de flecha para que se vea igual al de Registro --}}
                            <div
                    ...
                    <div class="mb-4">
                        ...
                        @if($producto->imagen_producto)
                            <div class="mb-3">
                                ...
                                <img src="{{ asset('storage/' . $producto->imagen_producto) }}" alt="Imagen actual" class="h-20 w-20 object-cover rounded-lg border border-gray-200">
                            </div>
                        @endif

                        {{-- El input SIN el atributo value --}}
                  ...
                  hover:file:bg-red-100 dark:text-gray-400">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Precio de Venta
                            <span class="text-red-500">*</span></label>
                        {{-- ... resto del input de precio ... --}}
                    </div>
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Fecha de Entrega
                        (Opcional)</label>
                    <input type="date" name="fecha_entrega" value="{{$producto->fecha_entrega}}"
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Descripción e
                        Ingredientes</label>
                    <textarea name="descripcion_producto" rows="4"
                              placeholder="Detalla los ingredientes..."
                              class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 outline-none transition-all dark:text-white resize-none">{{ $producto->descripcion_producto }}</textarea>
                </div>
            </div>
