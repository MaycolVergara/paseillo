...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    @foreach($usuarios as $usuario)
                        <tr class="group hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-all fila-paginada">
                            <td class="px-6 py-5">
                                <p class="text-[15px] font-extrabold text-gray-900 dark:text-white">{{ $usuario->nombre }}</p>
                            </td>
                            <td class="px-6 py-5">
                                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $usuario->correo }}</p>
                            </td>
                            <td class="px-6 py-5">
                                    <span
                            ...
                            <td class="px-6 py-5">
                                @php
                                    // Validamos si es admin o mozo para darle colores
                                    $esAdmin = $usuario->rol == 1;
                                ...
                                <span
                                    class="px-3 py-1 rounded-full text-[11px] font-black uppercase tracking-wider {{ $colorRol }}">
                                        {{ $usuario->rolAsignado ? $usuario->rolAsignado->rol : 'Sin rol' }}
                                    </span>
                            </td>
                                ...
                                <div class="inline-block text-left group/menu relative">
                                    ...
                                    <div
                                        ...
                                        <div class="p-1.5 space-y-1">
                                            ...
                                            <form
                                                action="{{ url('/dashboard/usuariosRegistro/'.$usuario->id_usuario) }}"
                                                method="POST"
        ...
        <div
            ...
            <form id="form-usuario" action="{{ url('/dashboard/usuariosRegistro') }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nombre" required placeholder="Ej. Maycol Vergara"
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="user" required placeholder="Ej. maycol123"
                           class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 outline-none transition-all dark:text-white font-medium">
                </div>
                ...
                <div class="mb-4">
                    ...
                    {{-- Nota: type="password" para que oculte los puntos --}}
                ...
                <div class="mb-6 relative">
                    ...
                    <select name="rol" required
                        ...
                        @foreach($rols as $rol)
                            <option value="{{ $rol->id_rol }}">{{ ucfirst($rol->rol) }}</option>
                        @endforeach
                    </select>
                    {{-- Flechita del select --}}
                    <div
