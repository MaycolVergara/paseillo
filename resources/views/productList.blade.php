...
@section('content')
    ...
    <div class="animate-in delay-1 bg-white dark:bg-gray-900 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-800 mb-6">
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-xl font-black text-gray-900 dark:text-white tracking-tight italic">Catálogo Paseillo</h2>
    ...
    {{-- Tabla Optimizada --}}
    <div class="animate-in delay-2 bg-white dark:bg-gray-900 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tr class="bg-gray-50/50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-700">
                    ...
                    <th class="px-6 py-4 text-[11px] font-black uppercase tracking-wider text-gray-400 text-center">Categoría</th>
                    <th class="px-6 py-4 text-[11px] font-black uppercase tracking-wider text-gray-400">Estado</th>
                    <th class="px-6 py-4 text-[11px] font-black uppercase tracking-wider text-gray-400 text-right">Acciones</th>
                </tr>
                </thead>
                ...
                @foreach($products as $product)
                    <tr class="group hover:bg-orange-50/30 dark:hover:bg-orange-900/5 transition-all fila-paginada">
                        ...
                        <td class="px-6 py-4">
                            {{-- precio -> price --}}
                            <span class="text-sm font-black text-emerald-600 dark:text-emerald-400">S/ {{ number_format($product->price, 2) }}</span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wider {{ $product->category ? 'bg-orange-100 text-orange-600 dark:bg-orange-900/30' : 'bg-gray-100 text-gray-500' }}">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="w-1.5 h-1.5 rounded-full {{ $product->delivery_date ? 'bg-blue-500' : 'bg-amber-500' }}"></span>
                        ...
                        <td class="px-6 py-4 text-right relative">
                            <div class="inline-block text-left group/menu relative">
                                ...
                                <div class="absolute right-0 top-full mt-2 w-36 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl shadow-xl z-50 invisible group-hover/menu:visible opacity-0 group-hover/menu:opacity-100 transition-all duration-200 transform origin-top-right scale-95 group-hover/menu:scale-100 overflow-hidden">
                                    <div class="p-1.5 space-y-1">
                                        ...
                                        <form action="{{ url('/dashboard/products/' . $product->id . '/delete') }}" method="POST" class="block">
                                            @csrf @method('DELETE')
        ...
        {{-- Paginación --}}
        <div id="paginacion-contenedor" class="px-6 py-3 border-t border-gray-100 dark:border-gray-800 flex justify-center items-center bg-gray-50/30 dark:bg-gray-800/20">
        </div>
    </div>
@endsection
