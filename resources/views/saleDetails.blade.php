    ...
    <div class="space-y-6">
        ...
        <div class="relative bg-white dark:bg-gray-900 rounded-3xl shadow-sm border border-gray-100/50 dark:border-gray-800 overflow-hidden">
            ...
            <div class="px-7 py-5 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-orange-100 to-red-50 dark:from-gray-800 dark:to-gray-800 border border-orange-200/50 dark:border-gray-700 flex items-center justify-center shadow-inner">
                        <span class="text-2xl">📊</span>
                    </div>
                    <div>
                        <h2 class="text-xl font-black text-gray-900 dark:text-white tracking-tight">Registro de Ventas</h2>
                        <p class="text-xs font-semibold text-gray-400 mt-0.5">
                            @if($start_date && $end_date)
                                Reporte del
                                <span class="text-orange-500">{{ \Carbon\Carbon::parse($start_date)->format('d/m/Y H:i') }}</span>
                                al
                                <span class="text-orange-500">{{ \Carbon\Carbon::parse($end_date)->format('d/m/Y H:i') }}</span>
                            @else
                ...
                <form action="{{ url('/dashboard/saleDetails') }}" method="GET"
        ...
        @if($start_date && $end_date)
            ...
            @php
                ...
                $ventasSalon    = $sales->whereNotNull('table_id')->count();
                $ventasDelivery = $sales->whereNotNull('table_delivery_id')->count();
                $metodos        = $sales->groupBy('payment_method')->map->count()->sortDesc();
                $topMetodo      = $metodos->keys()->first() ?? '---';
            @endphp

            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">

                {{-- Total Recaudado --}}
                <div class="col-span-2 lg:col-span-1 relative bg-gradient-to-br from-orange-500 via-red-500 to-rose-600 rounded-3xl p-5 shadow-lg shadow-orange-500/20 overflow-hidden">
                    ...
                    <div class="relative z-10">
                        <div class="flex items-center gap-2 mb-3">
                            ...
                            <p class="text-[10px] font-black uppercase text-white/90 tracking-widest">Total Recaudado</p>
                        </div>
                ...
                <div class="bg-white dark:bg-gray-900 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-gray-800">
                    <div class="w-9 h-9 bg-blue-50 dark:bg-blue-900/20 rounded-2xl flex items-center justify-center mb-3 text-blue-500">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {{-- EFECTIVO--}}
                <div class="bg-white dark:bg-gray-900 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-gray-800">
                    <div class="w-9 h-9 bg-purple-50 dark:bg-purple-900/20 rounded-2xl flex items-center justify-center mb-3 text-purple-500">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/>
                        </svg>
                    </div>
                    <p class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Venta en Efectivo</p>
                    <p class="text-2xl font-black text-gray-900 dark:text-white mt-1">
                        <span class="text-sm font-bold text-gray-400 mr-0.5">S/</span>{{ number_format($cashPayment ?? 0, 2) }}
                    </p>
                </div>
                ...
                <div class="bg-white dark:bg-gray-900 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-gray-800">
                    ...
                    <p class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Venta en Yape</p>
                    <p class="text-2xl font-black text-gray-900 dark:text-white mt-1">
                        <span class="text-sm font-bold text-gray-400 mr-0.5">S/</span>{{ number_format($yapePayment ?? 0, 2) }}
                    </p>
                </div>
                ...
                <div class="bg-white dark:bg-gray-900 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-gray-800">
                    ...
                    <p class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Venta en Tarjeta</p>
                    <p class="text-2xl font-black text-gray-900 dark:text-white mt-1">
                        <span class="text-sm font-bold text-gray-400 mr-0.5">S/</span>{{ number_format($cardPayment ?? 0, 2) }}
                    </p>
                </div>

                {{-- Método predominante --}}
                <div class="bg-white dark:bg-gray-900 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-gray-800">
                    <div class="w-9 h-9 bg-emerald-50 dark:bg-emerald-900/20 rounded-2xl flex items-center justify-center mb-3 text-emerald-500">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/>
                        </svg>
                    </div>
                    <p class="text-[10px] font-bold uppercase text-gray-400 tracking-widest">Pago Frecuente</p>
                    <h2 class="text-lg font-black text-gray-900 dark:text-white mt-1">{{ strtoupper($topMetodo) }}</h2>
                </div>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-3xl shadow-sm border border-gray-100/80 dark:border-gray-800 overflow-hidden">
                ...
                <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                    ...
                    <span class="text-[11px] font-black text-orange-600 bg-orange-100/80 dark:bg-orange-900/30 px-3 py-1 rounded-xl uppercase tracking-widest">
                    ...
                    @forelse($sales as $sale)
                        @php
                            ...
                            $metodoBadge = match(strtolower($metodo)) {
                                'yape'   => ['label' => 'Yape',     'color' => 'bg-teal-50 text-teal-600 border border-teal-200 dark:bg-teal-900/20 dark:border-teal-800'],
                                'plin'   => ['label' => 'Plin',     'color' => 'bg-pink-50 text-pink-600 border border-pink-200 dark:bg-pink-900/20 dark:border-pink-800'],
                                'card'   => ['label' => 'Tarjeta',  'color' => 'bg-blue-50 text-blue-600 border border-blue-200 dark:bg-blue-900/20 dark:border-blue-800'],
                                'cash'   => ['label' => 'Efectivo', 'color' => 'bg-emerald-50 text-emerald-600 border border-emerald-200 dark:bg-emerald-900/20 dark:border-emerald-800'],
                                default  => ['label' => $metodo,    'color' => 'bg-gray-100 text-gray-500 border border-gray-200'],
                            };
                        @endphp
                        ...
                        <div class="hover:bg-orange-50/30 dark:hover:bg-gray-800/30 transition-colors">
                            ...
                            <span class="text-[10px] font-black px-2.5 py-1 rounded-lg
                                ...
                                <div class="col-span-2">
                            <span class="text-[10px] font-black px-2.5 py-1 rounded-lg {{ $metodoBadge['color'] }}">
                                {{ $metodoBadge['label'] }}
                            </span>
                                </div>
                            ...
                            <span class="text-[13px] font-black text-gray-900 dark:text-white">
                                ...
                                <div class="col-span-1 flex justify-center">
                                    <svg id="icon-{{ $sale->id }}" class="w-4 h-4 text-gray-400 transition-transform duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                            ...
                            <div id="detalle-{{ $sale->id }}" class="hidden px-6 pb-4">
                                <div class="bg-gray-50 dark:bg-gray-800/40 rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-700">
                                    <div class="px-4 py-2.5 bg-gray-100/70 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                                        <p class="text-[11px] font-black uppercase tracking-widest text-gray-500">Productos del pedido</p>
                                    </div>
                                    <table class="w-full text-left">
                                        <thead class="text-[10px] uppercase tracking-wider text-gray-400 bg-white/50 dark:bg-gray-800/30">
                                        <tr>
                                        ...
                                        @foreach($details as $detail)
                                            ...
                                            <tr class="hover:bg-white dark:hover:bg-gray-800/50 transition-colors">
                                                <td class="px-4 py-2.5 text-[12px] font-bold text-gray-800 dark:text-gray-200">
                                                    {{ $product ? $product->name : '— Producto eliminado —' }}
                                                </td>
                                                <td class="px-4 py-2.5 text-center">
                                                <span class="text-[11px] font-black bg-orange-100 dark:bg-orange-900/30 text-orange-600 px-2 py-0.5 rounded-lg">
                                                    x{{ $detail->quantity }}
                                                </span>
                                                </td>
                                                <td class="px-4 py-2.5 text-right text-[12px] text-gray-500 dark:text-gray-400 font-semibold">
                                                    S/ {{ number_format($detail->unit_price, 2) }}
                                                </td>
                                                <td class="px-4 py-2.5 text-right text-[12px] font-black text-gray-900 dark:text-white">
                                        ...
                                        <tr class="bg-orange-50/60 dark:bg-orange-900/10">
                                            <td colspan="3" class="px-4 py-2.5 text-[11px] font-black uppercase tracking-widest text-gray-400 text-right">Total</td>
                ...
                {{-- Footer total --}}
                @if($sales->count() > 0)
                    <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30 flex items-center justify-between">
                        ...
                        <div class="flex items-baseline gap-1.5">
                            ...
                            <span class="text-2xl font-black text-gray-900 dark:text-white tracking-tighter">{{ number_format($totalDay, 2) }}</span>
                        </div>
                    </div>
                @endif
